...
export function randomizer(R) {
  ...
  const outputs = ['dumbbell', 'ringer', 'starburst', 'abacus', 'multiply', 'gas', 'block', 'puzzle'];
    // colors
  let schemes = [
    ['ffffff', '111111', '222222'], // Mono
    ['4281a4','48a9a6','e4dfda','d4b483','c1666b'], // 90s beach
    ['e7e6f7','e3d0d8','aea3b0','827081','c6d2ed'], // blueberry
    ['5d2a42','fb6376','fcb1a6','ffdccc','fff9ec'], // german
    ['fcd0a1','b1b695','53917e','63535b','6d1a36'], // ?? compliment
    ['ff6700','ebebeb','c0c0c0','3a6ea5','004e98'], // merca
    ['5bc0eb','fde74c','9bc53d','c3423f','404e4d'], // lego
    ['161925','23395b','406e8e','8ea8c3','cbf7ed'], // moody blu
    ['55dde0','33658a','2f4858','f6ae2d','f26419'], // hydro classic
    ['dbf9f4','e6fdff','d9d7dd','b07bac','5f7367'], // dino
    ['330c2f','7b287d','7067cf','b7c0ee','cbf3d2'], // lilac
    ['588b8b','ffffff','ffd5c2','f28f3b','c8553d'], // orange county
    ['8daa91','788475','5e5d5c','453643','28112b'], // grapes
    ['a3e7fc','26c485','32908f','553a41','2f0601'], // mint chocolate
    ['0d3b66','faf0ca','f4d35e','ee964b','f95738'], // sunset
    ['0d1821','344966','e6aace','f0f4ef','bfcc94'], // bruiser
    ['34f6f2','78e3fd','d1f5ff','eef8ff','7d53de'], // barney
    // 9 color scheme
    ['eae4e9','fff1e6','fde2e4','fad2e1','e2ece9','bee1e6','f0efeb','dfe7fd','cddafd'], // baby
    // 10 color schemes
    ['007f5f','2b9348','55a630','80b918','aacc00','bfd200','d4d700','dddf00','eeef20','ffff3f'], // lizard
    ['f72585','b5179e','7209b7','560bad','480ca8','3a0ca3','3f37c9','4361ee','4895ef','4cc9f0'], // miami
    ['03071e','370617','6a040f','9d0208','d00000','dc2f02','e85d04','f48c06','faa307','ffba08'], // lava
    ['f94144','f3722c','f8961e','f9844a','f9c74f','90be6d','43aa8b','4d908e','577590','277da1'], // toyland
    ['012a4a','013a63','01497c','014f86','2a6f97','2c7da0','468faf','61a5c2','89c2d9','a9d6e5'], // blue mono
    ...
    ['54478c','2c699a','048ba8','0db39e','16db93','83e377','b9e769','efea5a','f1c453','f29e4c'], // candy
    ['b76935','a56336','935e38','815839','6f523b','5c4d3c','4a473e','38413f','263c41','143642'], // martian
  ];
  const scheme = R.random_choice(schemes);
  ...
  let cropped = (croppedInt > 2 && croppedInt < 8 && ['gas','puzzle'].includes(output)) || ('puzzle' === output && size < 15)
                  ? 'square'
                  : croppedInt > 1 && (('puzzle' === output) || ('gas' === output && size > 15))
                    ? 'swept'
                    : croppedInt === 0 && (['puzzle','gas','block','starburst'].includes(output) || (['multiply','abacus'].includes(output) && size > 30) || ('ringer' === output && size > 90))
                      ? 'mundi'
                      : 'none';
...
// Dots
export function drillPegs(ctx, config, canvasWidth, canvasHeight) {
    ...
    // the x starting point: 1/6 the width // adjusted for multiply
    let cx = config.output === 'multiply' 
              ? Math.ceil( window.innerWidth / 4) + (base * 0.75)
  ...
  function countryFill(country, num, config) {
  ...
  // TODO FIGURE OUT RESPONSIVENENSS
  // randomize points : TODO remove random
